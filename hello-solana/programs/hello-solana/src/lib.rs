...
pub mod hello_solana {
    ...
    // 包含一个名为 initialize 的指令，该指令创建一个新帐户 (NewAccount) 并用 u64 值对其进行初始化。
...
// Initialize<'info> ：结构体名称，带有生命周期参数 'info ，这是 Anchor 处理账户时的标准做法
// Initialize 结构体实现了 Accounts 特性，其中结构体中的每个字段都代表初始化指令所需的帐户。
